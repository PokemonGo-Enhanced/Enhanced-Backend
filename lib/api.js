...
class API {
  ...
  wrapInRequestEnvelope(_payloads) {
    const { locals } = this.app;
    const payloads = is.array(_payloads) ? _payloads : [_payloads];
    ...
    return proto.serialize(envelope, 'POGOProtos.Networking.Envelopes.RequestEnvelope');
  }
  ...
  request(envelope) {
    return request.post(this.app.locals.apiEndpoint, envelope);
  }
}
