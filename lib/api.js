...
var api = module.exports = {
  wrapRequestPayloads: (payloads) => {
    if (!_.isArray(payload)) payloads = [payloads];

    payloads = payloads.map((payload) => {
      return this.wrapInRequest(payload);
    });
    var envelope = {
      status_code: 2,
      request_id: getId(),
  ...
  searchPokestop: (fort) => {
    ...
    var envelope = this.wrapRequestPayloads(payload);
  ...
  releasePokemon: (pokemonOrId) => {
    ...
    return this.request(this.wrapRequestPayloads(payload));
  },
