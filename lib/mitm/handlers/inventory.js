...
module.exports = mitmServer => {
  mitmServer.addResponseHandler('GetInventory', (data, action) => {
    try {
      ...
      if (addedPokemons.length) console.log('Got', addedPokemons.length, ' +pokemons in the delta')
      if (deletedPokemons.length) console.log('Got', deletedPokemons.length, ' -pokemons in the delta')
      if (items.length) console.log('Got', items.length, 'items in the delta')
